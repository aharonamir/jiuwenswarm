...
2. **Capture read path (durable-only, drained, guarded).** Capture reads `history.jsonl` and never the forwarded event stream. Because history writes are asynchronous (`_WRITE_QUEUE` + daemon writer thread, `session_history.py:618-653`), the adapter drains the queue via the existing `_WRITE_QUEUE.join()` (`:841`) before extraction begins, then segments **through the most recent persisted `chat.request_completed` belonging to a non-`/workflow` request**. Round 1 said "wait on the write receipt for the capturing request", which was circular — `/workflow save` is itself a request, and its own completion is neither available nor the boundary wanted. Two guards: capture refuses if any `chat.tool_result` in the span carries `truncated: True`, and refuses if no qualifying `chat.request_completed` is found.
   ...
   **Amendment (2026-08-27):** the second guard as written refuses on nearly every real session. `chat.request_completed` is written by exactly one call site, `enqueue_history_request_completion` (`session_history.py:750`), invoked only from `symphony/evolution/session_consumer.py:116` inside `schedule_session_evolution_consume` — which early-returns unless `load_symphony_config().enabled` and `.evolution.enabled` are both true. It is Symphony's own internal bookkeeping event, reused here, not a marker the core chat pipeline (`interface_deep.py`/`interface_code.py`/`interface.py`) ever writes itself — grepped, no other writer exists. Confirmed empirically: 0 of 13 real sessions on disk (including a 101-event, 42-tool-call session) contain this event. Decided: segment.py falls back to the last `chat.final` / end of the durable history span when `chat.request_completed` is absent, and uses it as the boundary only when present (Symphony enabled). See Assumption 18 and Risk 17.
3. **Fixtures from real sessions.** Seven recorded event streams: linear, fan-out, dead-steps, react, ambiguous-provenance, no-tools, multi-task. ~~Plus team-mode, which was a scope gate...~~ **Amendment (2026-08-27):** team mode is cut from this MVP phase by explicit decision, not by the empirical gate described below. Assumption 15's open half (whether `read_team_history_records()` carries leader/teammate `chat.tool_call`/`chat.tool_result`, and whether it arrives truncated) is left unanswered and reopens whenever team-mode capture is scheduled.
...
11. **Adapter + dispatch.** `server/runtime/agent_adapter/workflow_slash.py` following `evolution_slash.py`'s shape, plus a dispatch line in `interface_deep.py` (`:10461` precedent). **Amendment (2026-08-27):** the `team_helpers.py` (`:1525` precedent) dispatch line is cut from this MVP phase — see item 3. Two files, not three, change outside the package for now.
...
**Amendment (2026-08-27):** superseded before the gate ran. Team mode is now cut from this MVP phase by explicit product decision rather than by the empirical outcome above — `T009a` and `T055a` are marked cut in `tasks.md`, not pending. The gate itself (does `read_team_history_records()` carry the leader/teammate tool events) is neither proven nor disproven; it reopens whenever team-mode capture is scheduled.
...
18. **[NEW, 2026-08-27]** `chat.request_completed` — the boundary event approach step 2 segments on — is not a core chat-pipeline event. Its only writer, `enqueue_history_request_completion` (`session_history.py:750`), is called solely from Symphony's `schedule_session_evolution_consume` (`symphony/evolution/session_consumer.py:116`), gated on `load_symphony_config().enabled and .evolution.enabled`. No other call site exists anywhere in `interface_deep.py`, `interface_code.py`, or `interface.py`. Empirically absent from all 13 real sessions inspected, Symphony-enabled or not observed either way in this environment. *Consequence:* segment.py cannot treat this event as reliably present — see amendment on approach step 2 and Risk 17. — source: `session_history.py`, `symphony/evolution/session_consumer.py`
...
17. **[NEW, 2026-08-27] The segmentation boundary event does not exist outside an optional subsystem.** `chat.request_completed` (approach step 2, Assumption 18) is written only when Symphony's evolution consumer is enabled, which is off in this dev environment and possibly in most deployments. Decided fallback: segment.py boundaries on the last `chat.final` / end of durable history when the event is absent, and on `chat.request_completed` when present. *Open:* the fallback boundary is unmeasured — a session with a trailing non-`/workflow` request whose `chat.final` never lands (error, timeout) could segment on the wrong span. Not yet covered by a fixture; worth attacking when `session_multi_task` and `session_no_tools` extraction tests are written.
